...
import {
  User, Truck, Building2, DollarSign, Lock, LogOut, ChevronRight, ChevronLeft,
  Edit, Save, X, Shield, Phone, AlertTriangle, Trash2
} from "lucide-react";
import { useNavigate } from "react-router-dom";

type Section = "personal" | "delivery" | "banking" | "earnings" | "security";
...
const DeliveryProfile = () => {
  ...
  useEffect(() => {
    if (profile) {
      setForm({ full_name: profile.full_name || "", phone: profile.phone || "", email: profile.email || "", emergency_contact: (profile as any).emergency_contact || "" });
  ...
  const fetchPartnerSettings = async () => {
    if (!profile) return;
    const { data } = await supabase.from("delivery_partner_settings").select("*").eq("partner_id", profile.id).maybeSingle();
    if (data) { setPartnerSettings(data); setVehicleType(data.vehicle_type || "bike"); }
  };

  const fetchBankDetails = async () => {
    if (!profile) return;
    const { data } = await supabase.from("bank_details").select("*").eq("user_id", profile.id).maybeSingle();
    if (data) { setBankDetails(data); setBankForm({ account_holder_name: data.account_holder_name, bank_name: data.bank_name, account_number: data.account_number, ifsc_code: data.ifsc_code, upi_id: data.upi_id || "" }); }
  };

  const fetchPayouts = async () => {
    if (!profile) return;
    const { data } = await supabase.from("payouts").select("*").eq("recipient_id", profile.id).order("created_at", { ascending: false }).limit(20);
  ...
  const saveProfile = async () => {
    if (!profile) return;
    setSaving(true);
    await supabase.from("profiles").update({ full_name: form.full_name, phone: form.phone, email: form.email, emergency_contact: form.emergency_contact }).eq("id", profile.id);
    setSaving(false);
  ...
  const saveVehicle = async () => {
    ...
    setSaving(true);
    if (partnerSettings) {
      await supabase.from("delivery_partner_settings").update({ vehicle_type: vehicleType }).eq("partner_id", profile.id);
    } else {
      await supabase.from("delivery_partner_settings").insert([{ partner_id: profile.id, vehicle_type: vehicleType }]);
    }
    setSaving(false);
  ...
  const saveBankDetails = async () => {
    ...
    setSaving(false);
    toast({ title: "Bank details saved! Admin will verify changes." });
  ...
  const sections = [
    ...
    { id: "delivery" as Section, label: "DELIVERY", icon: Truck },
    { id: "banking" as Section, label: "BANKING", icon: Building2 },
    { id: "earnings" as Section, label: "EARNINGS", icon: DollarSign },
    { id: "security" as Section, label: "SECURITY", icon: Shield },
  ];

  const initials = (profile?.full_name || "D").split(" ").map((w: string) => w[0]).join("").toUpperCase().slice(0, 2);
  const totalEarnings = payouts.filter(p => p.status === "completed").reduce((a, p) => a + Number(p.amount || 0), 0);
    ...
    <div className="min-h-screen bg-background">
      <nav className="sticky top-0 z-50 bg-card/95 backdrop-blur-md border-b border-border shadow-soft">
        <div className="max-w-3xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate("/delivery")} className="p-2 rounded-full hover:bg-secondary"><ChevronLeft className="w-5 h-5" /></button>
      ...
      <div className="section-pink py-8 px-4">
        <div className="max-w-2xl mx-auto flex items-center gap-4">
          ...
          <div>
            <h1 className="font-heading text-2xl font-bold uppercase tracking-tight text-foreground">{profile?.full_name || "RIDER"}</h1>
      ...
      <div className="max-w-2xl mx-auto px-4 py-4">
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {sections.map((s) => (
            <button key={s.id} onClick={() => setSection(s.id)} className={`flex items-center gap-2 px-3 py-2 rounded-full font-heading font-bold text-xs uppercase tracking-wider whitespace-nowrap transition-all ${section === s.id ? "bg-primary text-primary-foreground shadow-soft" : "bg-card text-muted-foreground hover:bg-secondary"}`}>
      ...
      <div className="max-w-2xl mx-auto px-4 pb-24">
        {section === "personal" && (
          <div className="bg-card rounded-2xl p-6 shadow-card">
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-heading text-lg font-bold uppercase tracking-tight">PERSONAL INFO</h2>
              {!editing ? (
            ...
            {editing ? (
              <div className="space-y-3">
                ...
                <div><label className="text-xs text-muted-foreground uppercase tracking-wider font-heading font-bold">Emergency Contact</label><Input value={form.emergency_contact} onChange={(e) => setForm({ ...form, emergency_contact: e.target.value })} placeholder="Emergency phone number" className="rounded-xl bg-secondary border-0 mt-1" /></div>
              </div>
            ) : (
              <div className="space-y-3">
                {[{ label: "NAME", value: profile?.full_name }, { label: "PHONE", value: profile?.phone }, { label: "EMAIL", value: profile?.email }, { label: "EMERGENCY", value: (profile as any)?.emergency_contact }].map((item) => (
        ...
        {section === "delivery" && (
          <div className="bg-card rounded-2xl p-6 shadow-card">
            <h2 className="font-heading text-lg font-bold uppercase tracking-tight mb-4">VEHICLE INFO</h2>
            <div className="space-y-3">
              <label className="text-xs text-muted-foreground uppercase tracking-wider font-heading font-bold">Vehicle Type</label>
              <div className="flex gap-2">
                {["bike", "bicycle", "scooter", "car"].map((v) => (
                  <button key={v} onClick={() => setVehicleType(v)} className={`px-4 py-2 rounded-full font-heading font-bold text-xs uppercase tracking-wider ${vehicleType === v ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground"}`}>{v}</button>
                ))}
              </div>
              <Button onClick={saveVehicle} disabled={saving} className="rounded-full font-heading font-bold text-xs uppercase tracking-wider mt-2">SAVE VEHICLE</Button>
            </div>
          </div>
        )}
        ...
        {section === "banking" && (
          <div className="bg-card rounded-2xl p-6 shadow-card">
            <div className="flex justify-between items-center mb-4">
              ...
              <Button size="sm" variant="outline" onClick={() => setShowBankForm(!showBankForm)} className="rounded-full font-heading font-bold text-xs uppercase tracking-wider">{bankDetails ? "EDIT" : "ADD"}</Button>
            </div>
            <p className="text-xs text-muted-foreground mb-4 bg-secondary/50 p-3 rounded-xl flex items-center gap-2"><AlertTriangle className="w-4 h-4 text-primary" /> Banking edits require admin verification.</p>
        ...
        {section === "earnings" && (
          <div className="space-y-4">
            <div className="bg-card rounded-2xl p-6 shadow-card text-center">
              ...
              <p className="font-heading text-3xl font-bold text-primary">₹{totalEarnings}</p>
            </div>
            <h3 className="font-heading font-bold text-sm uppercase tracking-wider">PAYOUT HISTORY</h3>
            {payouts.length === 0 ? (
              <div className="text-center py-8 bg-card rounded-2xl shadow-card">
                <p className="text-muted-foreground uppercase tracking-wider text-sm">NO PAYOUTS YET</p>
              </div>
            ) : payouts.map((p) => (
              <div key={p.id} className="bg-card rounded-2xl p-4 shadow-card flex justify-between items-center">
                <div>
                  <p className="font-heading font-bold text-sm">₹{p.amount}</p>
                  <p className="text-xs text-muted-foreground">{new Date(p.created_at).toLocaleDateString()}</p>
                </div>
                <span className={`text-xs font-heading font-bold px-2 py-1 rounded-full uppercase ${p.status === "completed" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>{p.status}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {section === "security" && (
          <div className="space-y-4">
            ...
            <div className="bg-card rounded-2xl shadow-card overflow-hidden">
              <button onClick={signOut} className="w-full flex items-center gap-3 p-4 border-b border-border hover:bg-secondary/50 transition-colors">
                <LogOut className="w-5 h-5 text-destructive" />
                <span className="font-heading font-bold text-sm uppercase tracking-wider text-destructive">LOGOUT</span>
              </button>
              <button onClick={() => toast({ title: "Account deletion requested", description: "Contact admin to complete." })} className="w-full flex items-center gap-3 p-4 hover:bg-destructive/5 transition-colors">
